  ...
  try {
    ...
    if (action === "register") {
      ...
      const user = await upsertUser(env.DB, {
        uid: makeUID(), username, email, password, provider: "local",
        coins: 50, vip: false, role: "user", createdAt: Date.now()
      });
    ...
    if (action === "social") {
      ...
      let row = null;
      if (email) row = await env.DB.prepare("SELECT * FROM users WHERE lower(email)=? LIMIT 1").bind(email.toLowerCase()).first();

      const uid = row?.uid || makeUID();
      const user = await upsertUser(env.DB, {
        ...(row ? normalizeUser(row) : {}),
        uid, username, email, password: row?.password || "", provider, avatar,
        coins: row?.coins || 0, vip: !!row?.vip, role: row?.role || "user",
