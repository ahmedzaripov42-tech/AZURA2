...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    if (request.method === "POST") {
      ...
      if (bytes.length > 20 * 1024 * 1024) {
        return json({
          ...
          message: "20MB dan katta video uchun direct R2 upload kerak. Hozircha videoni 20MB dan kichik qiling yoki assets/CDN link ishlating."
        }, 413, cors());
      }
      ...
      await env.MEDIA.put(key, bytes, {
        ...
        customMetadata: { originalName: filename, uploadedAt: String(Date.now()) }
      });
    ...
    return json({ ok: false, error: "Media API error", message: String(err && err.message ? err.message : err) }, 500, cors());
  }
}
