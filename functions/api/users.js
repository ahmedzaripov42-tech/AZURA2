import { json, readJSON, ensureSchema, normalizeUser, upsertUser, OWNER_ID } from "./_common.js";
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    if (request.method === "PATCH") {
      ...
      if (action === "coins") await env.DB.prepare("UPDATE users SET coins=?, updatedAt=? WHERE uid=?").bind(Math.max(0, Number(body.coins || 0)), Date.now(), uid).run();
      else if (action === "vip") await env.DB.prepare("UPDATE users SET vip=?, updatedAt=? WHERE uid=?").bind((body.vip === undefined ? !u.vip : !!body.vip) ? 1 : 0, Date.now(), uid).run();
      else if (action === "role") {
        if (uid === OWNER_ID) return json({ ok:false, error:"Owner rolini o‘zgartirib bo‘lmaydi" }, 400);
        await env.DB.prepare("UPDATE users SET role=?, updatedAt=? WHERE uid=?").bind(["admin","user"].includes(body.role) ? body.role : "user", Date.now(), uid).run();
      } else return json({ ok:false, error:"Unknown action" }, 400);
      return json({ ok:true, user: normalizeUser(await env.DB.prepare("SELECT * FROM users WHERE uid=?").bind(uid).first()) });
    }
