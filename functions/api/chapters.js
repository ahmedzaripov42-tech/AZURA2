...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    if (request.method === "POST") {
      ...
      for (const c of list) {
        ...
        const t = Date.now(), id = String(c.id || `${c.manhwaId}-${c.chapterNo || c.number || t}`).trim();
        await env.DB.prepare(`INSERT INTO chapters (id, manhwaId, title, chapterNo, pages, accessType, price, vip, status, createdAt, updatedAt, extra) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET manhwaId=excluded.manhwaId,title=excluded.title,chapterNo=excluded.chapterNo,pages=excluded.pages,accessType=excluded.accessType,price=excluded.price,vip=excluded.vip,status=excluded.status,updatedAt=excluded.updatedAt,extra=excluded.extra`)
          .bind(id, String(c.manhwaId), String(c.title || c.name || ("Bob " + (c.chapterNo || c.number || ""))), Number(c.chapterNo || c.number || 0), JSON.stringify(c.pages || c.images || c.pageImages || []), String(c.accessType || c.access || "free"), Number(c.price || c.coin || c.coinPrice || 0), c.vip || c.vipOnly ? 1 : 0, String(c.status || "published"), Number(c.createdAt || t), t, JSON.stringify(c.extra || {})).run();
  ...
  } catch (err) { return json({ ok:false, error:"Chapters API error", message:String(err && err.message ? err.message : err) }, 500); }
}
