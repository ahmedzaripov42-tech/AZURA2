  ...
  try {
    ...
    if (request.method === "GET") {
      ...
      if (!key) {
        ...
        const data = {}, meta = {};
        for (const r of rows.results || []) { try { data[r.key] = JSON.parse(r.value); } catch { data[r.key] = r.value; } meta[r.key] = Number(r.updatedAt || 0); }
      ...
      let value = row.value; try { value = JSON.parse(row.value); } catch (_) {}
    ...
    if (request.method === "POST") {
      ...
      await env.DB.prepare(`INSERT INTO app_data (key, value, updatedAt) VALUES (?, ?, ?) ON CONFLICT(key) DO UPDATE SET value=excluded.value, updatedAt=excluded.updatedAt`).bind(key, value, t).run();
